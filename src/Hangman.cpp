...
bool hangMan:: play()
	...
	while(guesses>0)
		...
		for(int word = 0; word<letterWords.size(); word++)
			...
			for(int i=0; i<letterWords[word].length(); i++)
				...
				if(tempValueVec.size() > largestValueSize)
					{
					largestValueSize = tempValueVec.size();
					largestKey.clear();
					for(int l = 0; l<tempKeyVec.size(); l++)
						{
						largestKey.push_back(tempKeyVec[l]);
						}
					}
				}
		cout << families << endl;
		cout << "LARGEST KEY" << endl;
		for(int i=0; i<largestKey.size(); i++)
		{
			cout << largestKey[i];
			cout << endl;
		}
	...
	cout << "You finished at: " << current << endl;
	cout << "The word was " << families[lastWord];
	return false;
	}
